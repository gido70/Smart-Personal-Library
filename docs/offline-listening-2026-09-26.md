...
## Cloud sync boundary
Client outbox and sync adapter implemented. Applied additive SQL is docs/offline-listening-schema-proposal.sql; Applied after explicit user authorization on 2026-09-26. No existing table or RLS policy altered. Dedicated owner-scoped table and security-invoker RPC apply newer timestamps only. Cloud table and RPC are now deployed; the preview can sync on reconnection. Database transaction test passed: authenticated save, stale-write rejection, owner mismatch rejection, then rollback; zero test rows remained. RLS enabled, anonymous RPC access denied, security-invoker confirmed. Follow-up policy rejects anonymous sign-ins; authenticated owner writes and stale-write protection passed again, anonymous reads/writes were denied, and the security advisor reports no listening-table findings. Multi-device end-to-end sync is NOT verified. Client device-clock skew can prevent sync; retained dirty entries are not discarded.
...
Record OS/browser/version, preview URL and build. Prepare two real books online; confirm all parts and covers. Reorder trip; airplane mode; fully close/reopen; play all parts and cross-book transition; lock screen and switch apps. Reconnect and confirm cloud position through the deployed sync table/RPC. Delete local copy and confirm cloud originals/outputs survive. Test quota/failed preparation/update prompt. Verify old download controls unchanged. Test cover speed and author index separately. Do not infer device success from mocks.

## Open findings carried forward
Production author differs from retained printed-catalogue evidence: stored أحمد بن سالم بامهام vs باهمام، أحمد سالم عمر. Classification 610 saved. User confirmed covers now display excellently on Samsung and iPhone; existing cover loading was left unchanged. New offline-cover copies still require device acceptance. Notifications and PDF first pages need hardware/user-flow checks. Version file still 0.10.5-candidate, not a verified V0.11 label.
