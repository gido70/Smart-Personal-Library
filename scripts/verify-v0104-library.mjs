...
check("active cover thumbnails use a deterministic owner-scoped cache path", /activeCoverThumbnailPath/.test(app) && /ACTIVE_COVER_FILENAME/.test(app) && /cover-v2\.jpg/.test(read("src/lib/pdfAssets.ts")) && /saveCoverThumbnail/.test(library));
...
check("service worker cache advances to the cover-decoder fix", /smart-personal-library-v0\.10\.5-cover-decoders-5/.test(worker));
