...
type PaidAction = "process" | "ask" | "audio";
...
function errorText(error: unknown, rtl: boolean) {
  ...
  const known: Record<string, [string, string]> = {
    ...
    SUPABASE_NOT_CONFIGURED: ["اتصال المكتبة غير مُعدّ في هذه النسخة. لم يتم الاتصال بأي قاعدة بيانات.", "Library connection is not configured in this build. No database connection was attempted."],
...
function BookWorkspace({ rtl, book, onBack, onRead, onChanged }: { rtl: boolean; book: PilotBook; onBack: () => void; onRead: (page?: number) => void; onChanged: () => Promise<void> }) {
  ...
  const [confirming, setConfirming] = useState<PaidAction | "">("");
  const output = book.output_language === "en" ? "en" : "ar";
  const sizeMb = Math.max(0.1, (book.file_size || 0) / 1_048_576);
  const estimateBand = sizeMb < 5 ? "small" : sizeMb < 20 ? "medium" : "large";
  const estimates = {
    small: { process: "$0.05–$0.35", audio: "$0.10–$0.60" },
    medium: { process: "$0.20–$0.90", audio: "$0.10–$0.60" },
    large: { process: "$0.50–$1.80", audio: "$0.10–$0.60" },
  }[estimateBand];
  ...
  const runPaid = async (action: PaidAction, task: () => Promise<void>) => {
    await run(action, task);
    setConfirming("");
  };
  ...
  return <div className="v11-page"><button className="text-button" onClick={onBack}>→ {rtl ? "العودة إلى مكتبتي" : "Back to library"}</button><header className="v11-book-title"><div><small>{rtl ? "مسار كتابك" : "Your book journey"}</small><h1>{book.title}</h1><p>{rtl ? "كل مرحلة تقول لك بوضوح: لم تبدأ، جارية، أو مكتملة." : "Every stage is clearly marked as not started, in progress, or complete."}</p></div><button className="primary" onClick={() => onRead(progress?.page)}>{progress ? (rtl ? `تابع من ص ${progress.page}` : `Continue from p. ${progress.page}`) : (rtl ? "ابدأ القراءة" : "Start reading")}</button></header><section className="v11-journey-strip">{stages.map(([label, done], index) => <div key={label} className={done ? "done" : "pending"}><b>{done ? "✓" : index + 1}</b><span>{label}</span><small>{done ? (rtl ? "مكتمل" : "Complete") : (rtl ? "لم يبدأ" : "Not started")}</small></div>)}</section>{error && <div className="v11-error">{error}</div>}<div className="v11-work-grid">
    ...
    <section className="v11-panel v11-paid"><small>{rtl ? "AI اختياري" : "OPTIONAL AI"}</small><h2>{rtl ? "التحليل والملخص" : "Analysis and summary"}</h2>{paid ? <><p>{rtl ? "الملخص محفوظ ولن نعيد دفع تكلفته عند الفتح." : "The summary is saved and reused without paying again."}</p><details><summary>{rtl ? "عرض الملخص" : "Show summary"}</summary><p className="v11-summary">{summary}</p></details></> : <p>{ZERO_COST_MODE ? (rtl ? "مقفل الآن أثناء تجهيز V0.11." : "Locked while V0.11 is being prepared.") : (rtl ? `لن يبدأ قبل تأكيدك. التقدير المبدئي: ${estimates.process}.` : `Starts only after confirmation. Initial estimate: ${estimates.process}.`)}</p>}{!paid && !ZERO_COST_MODE && (confirming === "process" ? <PaidConfirmation rtl={rtl} action="process" estimate={estimates.process} busy={busy === "process"} onCancel={() => setConfirming("")} onConfirm={() => runPaid("process", async () => { await invokeBookAI(book.id, "process", { language: output }); })} /> : <button className="primary" disabled={busy === "process"} onClick={() => setConfirming("process")}>{rtl ? "راجع التكلفة" : "Review cost"}</button>)}</section>
    <section className="v11-panel"><small>{rtl ? "الصوت الاحترافي" : "PRO AUDIO"}</small><h2>{rtl ? "استمع إلى الملخص" : "Listen to the summary"}</h2>{audioUrls.length ? <div className="v11-audio-list">{audioUrls.map((url) => <audio key={url} controls preload="metadata" src={url}>Audio</audio>)}</div> : <p>{rtl ? `الصوت يُنشأ من الملخص فقط. التقدير المبدئي: ${estimates.audio}.` : `Audio is generated from the summary only. Initial estimate: ${estimates.audio}.`}</p>}{!audioUrls.length && paid && !ZERO_COST_MODE && (confirming === "audio" ? <PaidConfirmation rtl={rtl} action="audio" estimate={estimates.audio} busy={busy === "audio"} onCancel={() => setConfirming("")} onConfirm={() => runPaid("audio", async () => { await invokeBookAI(book.id, "audio", { language: output, voice: "marin" }); })} /> : <button className="primary" disabled={busy === "audio"} onClick={() => setConfirming("audio")}>{rtl ? "راجع تكلفة الصوت" : "Review audio cost"}</button>)}<p className="v11-note">{rtl ? "لا يُعتمد الصوت نهائيًا إلا بعد اختبار نطق عربي فعلي على الهاتف والكمبيوتر." : "Final voice acceptance requires a real Arabic pronunciation test on phone and desktop."}</p></section>
    <section className="v11-panel"><small>{rtl ? "اسأل الكتاب" : "ASK THE BOOK"}</small><h2>{rtl ? "سؤال من محتوى الكتاب" : "Ask from the book"}</h2><textarea value={question} onChange={(event) => { setQuestion(event.target.value); if (confirming === "ask") setConfirming(""); }} placeholder={rtl ? "اكتب سؤالك…" : "Type your question…"} />{confirming === "ask" ? <PaidConfirmation rtl={rtl} action="ask" busy={busy === "ask"} onCancel={() => setConfirming("")} onConfirm={() => runPaid("ask", async () => { const data = await invokeBookAI(book.id, "ask", { language: output, question }); setAnswer(data.answer); setQuestion(""); })} /> : <button className="secondary" disabled={ZERO_COST_MODE || !paid || !question.trim() || busy === "ask"} onClick={() => setConfirming("ask")}>{rtl ? "راجع التكلفة وأرسل" : "Review cost & ask"}</button>}{answer && <div className="v11-answer">{String(answer.answer ?? "")}</div>}{bundle?.questions.slice(0, 3).map((item) => <details key={item.id}><summary>{item.question}</summary><p>{String(item.answer?.answer ?? "")}</p></details>)}</section>
    <ReminderPanel rtl={rtl} book={book} reminder={reminder} onSaved={reload} />
  </div></div>;
}

function PaidConfirmation({ rtl, action, estimate, busy, onCancel, onConfirm }: { rtl: boolean; action: PaidAction; estimate?: string; busy: boolean; onCancel: () => void; onConfirm: () => void }) {
  const label = action === "process" ? (rtl ? "التحليل" : "analysis") : action === "audio" ? (rtl ? "الصوت" : "audio") : (rtl ? "السؤال" : "question");
  return <div className="v11-cost-confirm" role="alert"><strong>{rtl ? "تأكيد خدمة مدفوعة" : "Confirm paid service"}</strong><p>{rtl ? `سيستخدم ${label} رصيد OpenAI API الفعلي.${estimate ? ` التقدير: ${estimate}.` : " كل سؤال له تكلفة مستقلة."}` : `${label} will use actual OpenAI API credit.${estimate ? ` Estimate: ${estimate}.` : " Each question has a separate cost."}`}</p><div><button className="primary" disabled={busy} onClick={onConfirm}>{busy ? (rtl ? "جارٍ التنفيذ…" : "Working…") : (rtl ? "أوافق وابدأ" : "Confirm and start")}</button><button className="secondary" disabled={busy} onClick={onCancel}>{rtl ? "تراجع" : "Go back"}</button></div></div>;
}
